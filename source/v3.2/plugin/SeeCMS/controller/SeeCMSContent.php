...
class SeeCMSContentController {
      ...
      for( $set = 0; $set < $sets; $set++ ) {
        ...
        foreach( $field as $f ) {
          ...
          if( $fd[2] == 'text' ) {
            ...
            $o .= "<input type=\"text\" name=\"adf{$contentContainerID}-field{$fd[0]}-set{$set}\" value=\"{$content[$set][$fd[0]]}\" />";
          }
      ...
      if( $settingsA['repeatable'] == 'true' ) {
        ...
        foreach( $field as $f ) {
          ...
          if( $fd[2] == 'image' ) {
            ...
            $o2 .= "<a class=\"adfselectimage\" id=\"adf{$contentContainerID}-field{$fd[0]}-setSEECMSADFSET\">Select image</a><div class=\"adf{$contentContainerID}-field{$fd[0]}-setSEECMSADFSET\"></div>";
  ...
  public function makeEditPart( $contentContainerID, $contentType, $content, $status, $settingsScreen = false ) {
    ...
    if( !$settingsScreen ) {
      $o .= "<div class=\"editable\"><div class=\"editable{$contentContainerID}\">{$content}</div><p class=\"editbar\"><a class=\"editcontent editcontent".ucwords(str_replace(" ", "", $contentType))."\" id=\"editable{$contentContainerID}\">Edit</a>";
        
      if( $status ) {
        $o .= " | This content is live";
      } else {
  ...
  public function loadForLinkSelector( $html = 0, $visible = 0 ) {
    ...
    if( $html ) {
      $o .= "<div class=\"adflinks\"".((!$visible)?" style=\"display: none;\"":"")."><div class=\"select adfselectLink\"><p><select id=\"linktype\"><option value=\"\">Select link type</option><option value=\"page\">CMS page</option><option value=\"post\">CMS post</option><option value=\"download\">CMS download</option><option value=\"email\">Email</option><option value=\"external\">External link</option></select></p></div><div class=\"hidden\" style=\"display: block;\"><div class=\"folders pages\"".(($visible)?" style=\"display: none;\"":"").">{$data['pages']}</div><div class=\"folders posts\"".(($visible)?" style=\"display: none;\"":"").">{$data['posts']}</div><div class=\"folders emails\"".(($visible)?" style=\"display: none;\"":"")."><p>Insert email address</p><p><input type=\"text\" id=\"emaillink\" /></p></div><div class=\"folders externals\"".(($visible)?" style=\"display: none;\"":"")."><p>Insert full url</p><p><input type=\"text\" id=\"weblink\" value=\"http://\" /></p></div><div class=\"folders downloads\"".(($visible)?" style=\"display: none;\"":"").">{$data['downloads']}</div></div><div class=\"clear\"></div></div>";
