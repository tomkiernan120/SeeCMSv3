...
class SeeImageController {
  ...
  public function prepare( $image, $newImage, $width, $height, $ext, $constrain = false, $stretch = false ) {
    ...
    if( $newImg ) {
      $newImg['status'] = $this->save( $newImg['img'], $newImage );
      return( $newImg );
    } else {
      $newImg['status'] = false;
      return( $newImg );
  ...
  private function save( $img, $filename ) {
