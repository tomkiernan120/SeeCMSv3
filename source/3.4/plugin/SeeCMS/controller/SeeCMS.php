...
class SeeCMSController {
  ...
  private function dynamicRouteManager( $r ) {
    ...
    if( $this->see->customContentLoading[ $r->objecttype ] ) {
      ...
      $content = SeeDB::find( 'content', ' objecttype = ? && objectid = ? && language = ? && status IN ( '.$status.' ) ORDER BY contentcontainer_id ASC, status ASC ', array( $r->objecttype, $r->objectid, $this->language ) );
  ...
  public function http404() {
    ...
    if ( preg_match( '/^images\/uploads\/img-([0-9]{1,10})-([0-9]{1,5})\.(jpg|png|gif)$/', $route, $img )) {
