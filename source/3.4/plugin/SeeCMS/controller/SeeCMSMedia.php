...
class SeeCMSMediaController {
  ...
  public function loadByFolder( $parentID = 0, $mode = 'admin' ) {
    ...
    foreach( $media as $m ) {
      ...
      if( $mode == 'selectimage' ) {
        ...
        $_SESSION['SeeCMS'][$this->see->siteID]['media']['currentFolder'] = $parentID;
        if( $m->type == 'mp4' ) {
          $content .= "<div class=\"thumb\"><div class=\"overlay\"><p>{$m->name}</p><p>{$m->type}</p><a href=\"#\" id=\"move{$m->id}\" class=\"move\">Move</a><a href=\"edit/?id={$m->id}\" class=\"viewedit\">View/Edit</a><a id=\"deletemedia-{$m->id}\" class=\"delete deletemedia\">Delete</a></div><img src=\"../../seecms/images/vid.png\" alt=\"\" /></div>";
        } else {
  ...
  public function selectimageOptions() {
    ...
    $data['imagesizes'] = $is = SeeDB::find( 'imagesize', ' ( theme = ? || theme = ? ) && selectable = ? ORDER BY name ', array( '', $this->see->theme, 1 ) );
