...
class SeeCMSPostController {
  ...
  public function update( $data, $errors, $settings ) {
    ...
    if( $p->id ) {
      ...
      $p->tags = $data['tags'];
      $p->date = $this->see->format->date( $data['postdate'], "Y-m-d" );
  ...
  private function recursiveDelete( $parentID ) {
    ...
    $posts = SeeDB::find( 'page', ' parentid = ? && deleted = ? ', array( $parentID, '0000-00-00 00:00:00' ) );
    foreach( $posts as $p ) {
      $this->delete( $p->id, 1 );
    }
  }
  ...
  public function loadByFolder( $parentID = 0 ) {
    ...
    $posts = SeeDB::find( 'post', ' parentid = ? && deleted = ? && isfolder = ? ORDER BY date DESC ', array( $parentID, '0000-00-00 00:00:00', 0 ) );
    foreach( $posts as $p ) {
      
      $date = $this->see->format->date( $p->date, "d.m.Y" );
  ...
  function feed( $settings = array() ) {
    ...
    if( is_array( $posts ) ) {
      foreach( $posts as $p ) {
        ...
        $ps[] = array( 'title' => $p->title, 'media' => $p->media, 'route' => "/{$route->route}", 'date' => $p->date, 'eventStart' => $p->eventstart, 'eventEnd' => $p->eventend, 'standfirst' => $p->standfirst, 'content' => $pscontent, 'tagsHTML' => $tagsHTML, 'tags' => $tags );
      }
    }
