...
$adfs = SeeDB::find( 'adf', ' objecttype = ? && ( ( objectid = ? && `cascade` = ? ) || ( objectid IN ( '.$page->ascendants.' ) && `cascade` = ? ) || ( objectid IN ( '.$page->ascendants.' ) && `cascade` = ? && objectid != ? ) ) && ( theme = ? || theme = ? ) ', array( 'page', $page->id, 0, 1, 2, $page->id, '', $see->theme ) );
