...
class SeeHTMLController {
  ...
  public function js( $fileOrArray = '', $script = '', $path = '/js/', $name = '', $override = false ) {
    ...
    if( !$skip ) {
      if( $snappy ) {
        $this->js = "<script type=\"text/javascript\"".(($file)?" src=\"{$path}{$file}{$versionParameter}\"":"").">{$script}</script>\n".$this->js;
      } else if( $late ) { 
        $this->jsLate .= "<script type=\"text/javascript\"".(($file)?" src=\"{$path}{$file}{$versionParameter}\"":"").">{$script}</script>\n";
      } else {
        $this->js .= "<script type=\"text/javascript\"".(($file)?" src=\"{$path}{$file}{$versionParameter}\"":"").">{$script}</script>\n";
      }
    }
  ...
  public function meta( $nameOrArray, $content = '' ) {
    ...
    if( is_array( $nameOrArray ) ) {
      $this->meta .= $this->makeTag( 'meta', true, $nameOrArray, true, true );
    } else {
