  ...
  public function update( $data, $errors, $settings ) {
    ...
    if( $p->title ) {
      ...
      if( strtolower( $p->title ) != strtolower( $data['title'] ) ) {
        ...
        // Make old primary route a secondary route
        SeeDB::exec( " UPDATE route SET primaryroute = 0 WHERE objecttype = 'page' && objectid = {$p->id} " );
  ...
  public function move( $id = 0, $at = '' ) {
    ...
    if( ( $pp->id || $at[1] === '0' ) && $pp->id != $id ) {
      ...
      if( $p->id ) {
        ...
        if( $oldOrder != $p->pageorder || $oldParent != $parentid ) {
        
          SeeDB::store( $p );
          
          SeeDB::exec( " UPDATE page SET pageorder = pageorder - 1 WHERE parentid = {$oldParent} && pageorder >= {$oldOrder} && deleted = '0000-00-00' " );
  ...
  public function navigation( $settings ) {
    ...
    foreach( $pages as $p ) {
      ...
      if( $settings['html'] ) {
        ...
        $page[] = array( 'title' => $p->title, 'route' => $route, 'selected' => $selected, 'subpages' => $subpages, 'id' => $id );
      }
    }
